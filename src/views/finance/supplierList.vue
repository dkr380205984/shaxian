    ...
    <div class="module">
      ...
      <div class="listCtn">
        ...
        <div class="list">
          ...
          <div class="bodyCtn">
            <div class="row" v-for="item in clientList" :key="item.client_id + '客户名称'">
              <div class="column">{{ item.client_name }}</div>
              <div class="column" v-if="openTShow">{{ $toFixed(item.total_weight / 1000, 2, true) || 0 }}吨</div>
              <div class="column" v-else>{{ $toFixed(item.total_weight, 2, true) || 0 }}kg</div>
              <div class="column" v-if="openWMoneyShow">{{ $toFixed(item.total_price / 10000, 2, true) || 0 }}万元</div>
              <div class="column" v-else>{{ $toFixed(item.total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openTShow">{{ $toFixed(item.real_total_weight / 1000, 2, true) || 0 }}吨</div>
              <div class="column" v-else>{{ $toFixed(item.real_total_weight, 2, true) || 0 }}kg</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(item.real_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.real_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(item.invoice_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.invoice_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(item.collection_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.collection_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(item.deduct_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.deduct_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(item.invoice_wait / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.invoice_wait, 2, true) || 0 }}元</div>
              <div class="column">
                <span class="col_btn blue" @click="$router.push('/finance/supplierDetail/' + item.client_id)"
                  >详情</span
                >
              </div>
            </div>
            <div class="row green">
              ...
              <div class="column" v-if="openTShow">{{ $toFixed(total_sts.total_weight / 1000, 2, true) || 0 }}吨</div>
              <div class="column" v-else>{{ $toFixed(total_sts.total_weight, 2, true) || 0 }}kg</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(total_sts.total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openTShow">
                {{ $toFixed(total_sts.real_total_weight / 1000, 2, true) || 0 }}吨
              </div>
              <div class="column" v-else>{{ $toFixed(total_sts.real_total_weight, 2, true) || 0 }}kg</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.real_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(total_sts.real_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.collection_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(total_sts.collection_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.invoice_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(total_sts.invoice_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.deduct_total_price / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(total_sts.deduct_total_price, 2, true) || 0 }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.invoice_wait / 10000, 2, true) || 0 }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(total_sts.invoice_wait, 2, true) || 0 }}元</div>
          ...
          <div class="buttonList">
            ...
            <div class="otherInfoCtn">
              <div class="otherInfo" style="width: 320px">
                ...
                <div class="btn backHoverBlue" @click="importExcelData('付款单据', '元')">
                  <svg class="iconFont" aria-hidden="true">
                    <use xlink:href="#icon-xiugaidingdan"></use>
                  </svg>
    ...
    saveImportData(data: any, type: string) {
      ...
      if (type === '开票单据') {
        ...
        submitData.forEach((item) => {
          ...
          if (!obj) {
            this.$message.error('解析失败，有所填单位非供货商单位全称，或者该单位不存在，请仔细检查')
            err = true
          } else {
