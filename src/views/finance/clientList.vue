      ...
      <div class="listCtn">
        <div class="filterCtn">
          <div class="leftCtn">
            ...
            <div class="elCtn">
              <el-checkbox v-model="openTShow">数量吨位</el-checkbox>
              <el-checkbox v-model="openWMoneyShow">金额万位</el-checkbox>
            </div>
          </div>
          <div class="rightCtn">
        ...
        <div class="list">
          ...
          <div class="bodyCtn">
            <div class="row" v-for="item in clientList" :key="item.id">
              ...
              <div class="column" v-if="openTShow">{{ $toFixed(item.total_weight / 1000, 2, true) }}吨</div>
              <div class="column" v-else>{{ $toFixed(item.total_weight, 2, true) }}kg</div>
              <div class="column" v-if="openWMoneyShow">{{ $toFixed(item.total_price / 10000, 2, true) }}万元</div>
              <div class="column" v-else>{{ $toFixed(item.total_price, 2, true) }}元</div>
              <div class="column" v-if="openTShow">{{ $toFixed(item.real_total_weight / 1000, 2, true) }}吨</div>
              <div class="column" v-else>{{ $toFixed(item.real_total_weight, 2, true) }}kg</div>
              <div class="column" v-if="openWMoneyShow">{{ $toFixed(item.real_total_price / 10000, 2, true) }}万元</div>
              <div class="column" v-else>{{ $toFixed(item.real_total_price, 2, true) }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed((item.invoice_total_price || 0) / 10000, 2, true) }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.invoice_total_price || 0, 2, true) }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed(item.collection_total_price / 10000 || 0, 2, true) }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.collection_total_price || 0, 2, true) }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed((item.deduct_total_price || 0) / 10000, 2, true) }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.deduct_total_price || 0, 2, true) }}元</div>
              <div class="column" v-if="openWMoneyShow">
                {{ $toFixed((item.invoice_wait || 0) / 10000, 2, true) }}万元
              </div>
              <div class="column" v-else>{{ $toFixed(item.invoice_wait || 0, 2, true) }}元</div>
              <div class="column">
            ...
            <div class="row">
              ...
              <div class="column green" v-if="openTShow">{{ $toFixed(total_sts.total_weight / 1000, 2, true) }}吨</div>
              <div class="column green" v-else>{{ $toFixed(total_sts.total_weight, 2, true) }}kg</div>
              <div class="column green" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.total_price / 10000, 2, true) }}万元
              </div>
              <div class="column green" v-else>{{ $toFixed(total_sts.total_price, 2, true) }}元</div>
              <div class="column green" v-if="openTShow">
                {{ $toFixed(total_sts.real_total_weight / 1000, 2, true) }}吨
              </div>
              <div class="column green" v-else>{{ $toFixed(total_sts.real_total_weight, 2, true) }}kg</div>
              <div class="column green" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.real_total_price / 10000, 2, true) }}万元
              </div>
              <div class="column green" v-else>{{ $toFixed(total_sts.real_total_price, 2, true) }}元</div>
              <div class="column green" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.collection_total_price / 10000, 2, true) }}万元
              </div>
              <div class="column green" v-else>{{ $toFixed(total_sts.collection_total_price, 2, true) }}元</div>
              <div class="column green" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.invoice_total_price / 10000, 2, true) }}万元
              </div>
              <div class="column green" v-else>{{ $toFixed(total_sts.invoice_total_price, 2, true) }}元</div>
              <div class="column green" v-if="openWMoneyShow">
                {{ $toFixed(total_sts.deduct_total_price / 10000, 2, true) }}万元
              </div>
              <div class="column green" v-else>{{ $toFixed(total_sts.deduct_total_price, 2, true) }}元</div>
              <div class="column green" v-if="openWMoneyShow">
                {{ $toFixed((total_sts.invoice_wait || 0) / 10000, 2, true) }}万元
              </div>
              <div class="column green" v-else>{{ $toFixed(total_sts.invoice_wait || 0, 2, true) }}元</div>
            ...
            <div class="buttonList">
              ...
              <div class="otherInfoCtn">
                <div class="otherInfo" style="width: 320px">
                  ...
                  <div class="btn backHoverRed" @click="importExcelData('扣款单据')">
                    <svg class="iconFont" aria-hidden="true">
                      <use xlink:href="#icon-xiugaidingdan"></use>
                    </svg>
                    <span class="text">扣款单据</span>
                  </div>
                  <div class="btn backHoverBlue" @click="importExcelData('收款单据', '元')">
                    <svg class="iconFont" aria-hidden="true">
                      <use xlink:href="#icon-xiugaidingdan"></use>
                    </svg>
            ...
            <div class="buttonList">
              ...
              <div class="otherInfoCtn">
                <div class="otherInfo" style="width: 320px">
                  ...
                  <div class="btn backHoverBlue" @click="downloadExcel('收款单据模板')">
                    <svg class="iconFont" aria-hidden="true">
                      <use xlink:href="#icon-xiugaidingdan"></use>
                    </svg>
                    <span class="text">收款单据</span>
...
<script lang="ts">
import Vue from 'vue'
import { finance, deduct, collection, bill } from '@/assets/js/api'
    ...
    saveImportData(data: any, type: string) {
      ...
      if (type === '开票单据') {
        ...
        typeObj = {
          ...
          desc: ['收款备注(选填)', ''],
          collect_or_pay: [false, 1],
          collect_type: [false, 5],
          file_url: [false, null],
          id: [false, null],
          pid: [false, []]
        }
      ...
      if (type === '开票单据') {
        ...
        submitData.forEach((item) => {
          ...
          if (!obj) {
            this.$message.error('解析失败，有所填单位非客户单位全称，或者该单位不存在，请仔细检查')
            err = true
          } else {
