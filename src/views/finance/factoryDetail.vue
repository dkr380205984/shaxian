<template>
  <div class="indexMain" id="clientDetail" v-loading="loading">
        ...
        <div class="tbody" style="font-weight: bold; font-size: 16px">
          <div class="trow">
            ...
            <div class="tcolumn red">
              {{
                $toFixed(
                  client_info.total_price - client_info.deduct_total_price - client_info.collection_total_price,
                  2,
                  true
                )
              }}元
                  ...
                  <div class="bodyCtn">
                    <div class="row" v-for="(item, index) in stock_list" :key="index + '加工'">
                      <div
                        ...
                        @click="$router.push('/directProcess/yarnDetailNew/' + item.transfer_id)"
                      >
                        {{ item.code }}
                      </div>
                      <div class="column">{{ item.type }}</div>
                      <div
                        class="column hoverBlue"
                      ...
                      <div class="column noPad" style="flex-direction: column; flex: 5">
                        ...
                        >
                          ...
                          <div class="column">
                            {{ $toFixed((Number(itemChild.weight) || 0) * (Number(itemChild.price) || 0), 2, true) }}
                          </div>
                        </div>
                      </div>
                      <div
                        class="column hoverBlue"
                        style="cursor: pointer"
                        @click="$openUrl('/print/processPrint?id=' + item.id)"
                      >
                        打印
                      </div>
                    </div>
    ...
    <bill :data="{ type: 2, client_id: $route.params.id, myType: 2 }" :show.sync="show_bill" @afterBill="init"></bill>
    <deduct
      :data="{ type: 2, client_id: $route.params.id, myType: 2 }"
      :show.sync="show_deduct"
      @afterDeduct="init"
    ></deduct>
    <collection
      :data="{ type: 2, client_id: $route.params.id }"
  ...
  methods: {
    init() {
      ...
      Promise.all([
        ...
        res[3].data.data.total_other_fee =
          res[3].data.data.others_fee?.reduce((a: any, b: any) => {
            return (
              a +
