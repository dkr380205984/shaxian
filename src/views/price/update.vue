          ...
          <div class="colCtn flex3">
            ...
            <div class="content">
              <el-input v-model="price_name"
                placeholder="请输入报价单名称(默认系统生成报价单编号)" />
            </div>
          </div>
          <div class="colCtn flex3">
            <div class="label">
...
export default Vue.extend({
  ...
  created() {
    Promise.all([
      ...
      // 初始化报价单修改数据
      const priceDetail = res[2].data.data
      this.price_name = priceDetail.title
      this.client_name = priceDetail.client_id
      this.order_code = priceDetail.order_id
      this.searchClientCode(priceDetail.order_code)
      this.remark = priceDetail.desc
      this.editInfo = priceDetail.child_data.map((itemM: any) => {
        const finded = this.yarnArr.find((itemF: any) =>
          itemF.children.find((itemFI: any) => itemFI.value === Number(itemM.yarn_id))
        )
        const obj = {
          yarn_name: [finded && finded.value, Number(itemM.yarn_id)],
          colorArr: [],
          color: itemM.color,
          attr: itemM.attribute,
          cost_price: itemM.cost_price,
          set_number: itemM.weight,
          ask_for_quality: itemM.mass_demand,
          ask_for_other: itemM.other_demand,
          process: (itemM.yarn_process_info && JSON.parse(itemM.yarn_process_info)) || [
            { name: '', desc: '', price: '' }
          ],
          other: (itemM.other_fee && JSON.parse(itemM.other_fee)) || [{ name: '', desc: '', price: '' }],
          profits: { rate: itemM.profit_proportion, price: itemM.profit },
          tax: { rate: itemM.tax_proportion, price: itemM.tax },
          total_price: itemM.total_price
        }
        this.selectYarnEvent(obj, [NaN, itemM.yarn_id])
        return obj
      })
    })
  },
